...
// MARK: - Camera Manager
// Uses @unchecked Sendable because AVCaptureSession is itself thread-safe.
// All @Observable property mutations are marshalled back to @MainActor via Task.
...
final class MirrorCameraManager: @unchecked Sendable {
    ...
    func setup() {
        ...
        case .authorized:
            Task { @MainActor [weak self] in self?.isAuthorized = true }
            startSession()
        case .notDetermined:
        ...
        default:
            Task { @MainActor [weak self] in self?.isAuthorized = false }
        }
    }
...
struct MirrorModeView: View {
    ...
    enum PostureState {
        ...
        var accessibilityLabel: String {
            switch self {
            case .checking: return "Checking posture. Align yourself with the alignment grid."
            case .good:     return "Good alignment. Open your shoulders a little more."
            case .perfect:  return "Perfect alignment achieved!"
            }
        }
    }
    ...
    var body: some View {
        ZStack {
            ...
            if camera.isAuthorized {
                CameraPreviewView(session: camera.session)
                    ...
                    .accessibilityLabel("Front camera preview showing your posture")

                AlignmentGridOverlay(
                    opacity: gridOpacity,
                    sparkle: postureState == .perfect && sparkle && !reduceMotion
                )
        ...
        .accessibilityLabel("Mirror Mode — posture alignment camera")
    }

    // MARK: - Sub-views

    private var postureFeedbackBadge: some View {
        HStack(spacing: 6) {
            Circle()
                .fill(postureState.color)
                .frame(width: 8, height: 8)
                .neonGlow(color: postureState.color, radius: 6)
                .scaleEffect(checkPulse && !reduceMotion ? 1.4 : 1.0)
                .accessibilityHidden(true)
        ...
        .clipShape(Capsule())
        .overlay(Capsule().stroke(postureState.color.opacity(0.6), lineWidth: 1.5))
        .neonGlow(color: postureState.color, radius: 8)
        .accessibilityLabel(postureState.accessibilityLabel)
    }

    private var checkAlignmentButton: some View {
        Button { checkPosture() } label: {
        ...
        .accessibilityLabel("Check my posture alignment")
        .accessibilityHint("Cycles through posture feedback states")
    }
    ...
    private var noCameraView: some View {
        VStack(spacing: 20) {
            ...
            .foregroundStyle(Color.pmPrimary)
            .accessibilityLabel("Open iOS Settings to grant camera access")
        }
    ...
    private var controlsOverlay: some View {
        VStack {
            HStack {
                ...
                .accessibilityLabel(showGuides ? "Hide alignment guides" : "Show alignment guides")
                .padding(.top, 68)
                .padding(.leading, 20)
...
private struct AlignmentGridOverlay: View {
    ...
    // Fixed corner definitions — use enum cases, not tuples, to avoid ForEach ID collisions
    private enum Corner: CaseIterable {
        case topLeft, topRight, bottomLeft, bottomRight
        var xFraction: CGFloat { self == .topRight || self == .bottomRight ? 1 : 0 }
        var yFraction: CGFloat { self == .bottomLeft || self == .bottomRight ? 1 : 0 }
        var xOffset:   CGFloat { xFraction == 0 ?  24 : -24 }
        var yOffset:   CGFloat { yFraction == 0 ?  80 : -24 }
    }
    ...
    var body: some View {
        GeometryReader { geo in
            ZStack {
                ...
                // Corner sparkles (perfect state only)
                if sparkle {
                    ForEach(Corner.allCases, id: \.self) { corner in
                        Image(systemName: "sparkle")
                            ...
                            .goldGlow(radius: 12)
                            .position(
                                x: geo.size.width  * corner.xFraction + corner.xOffset,
                                y: geo.size.height * corner.yFraction + corner.yOffset
                            )
                    }
                }
            }
